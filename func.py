...
# from .apiconfig import opt, ApiConfig
...
def recog_postprocess(result, class_result = None, need_address = True):
    ...
    if class_result == 'id':
        ...
        qualification = ApiConfig.qualification
        class_result = {'name':[],'id_number':[],'country':[],'issue_date':[],'qualification':[]}
        for i in result:
            if i == 'qualification':
                txt = ' '.join([a[0] for a in result[i] if a[0].strip() != ''])
                st, end =  txt.find('('), txt.find(')')
                if st == -1:
                    st = txt.find('-') - 1
                txt = txt[st+1:].upper() if end == -1 else txt[st+1:end].upper()
                if not qualification.get(txt):
                    temp = 999
                    class_ = ''
                    for b in qualification:
                        scr =  levenshtein(txt,b) - diff_ngram(txt,b,1)
                        if scr < temp:
                            temp = scr
                            class_ = b
                    txt = qualification[class_] + '('+class_+')'
                else:
                    txt = qualification[txt] +'('+ txt+')'
